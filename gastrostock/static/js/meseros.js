...
(function() {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (ubicacionSelect) {
            ubicacionSelect.addEventListener('change', function() {
                ...
                actualizarMesasPorUbicacion(ubicacionSeleccionada);
            });
        }
    });
    ...
    function actualizarMesasPorUbicacion(ubicacion) {
        const baseUrl = window.location.pathname;
        fetch(`${baseUrl}?ubicacion=${ubicacion}`, {
            method: 'GET',
            headers: {
                'X-Requested-With': 'XMLHttpRequest'
            }
    ...
    function iniciarActualizacionAutomatica() {
        const intervaloActualizacion = 10000;

        async function actualizarContenido() {
            try {
                let url = window.location.pathname;
                if (ubicacionSeleccionada) {
                    url += `?ubicacion=${ubicacionSeleccionada}`;
                }
        ...
        const intervalId = setInterval(actualizarContenido, intervaloActualizacion);
        window.actualizacionInterval = intervalId;
    }
    ...
    function actualizarElementoSiExiste(id, newDoc) {
        ...
        if (oldElement && newElement) {
            if (id === 'mesasContainer' && ubicacionSeleccionada) {
                return;
            }

            const elementosConEventos = oldElement.querySelectorAll('[data-pedido-id], [data-mesa-id]');
            const eventosPreservados = Array.from(elementosConEventos).map(el => ({
                dataset: { ...el.dataset },
            ...
            const nuevosElementos = oldElement.querySelectorAll('[data-pedido-id], [data-mesa-id]');
            nuevosElementos.forEach((el, index) => {
                if (eventosPreservados[index]) {
                    Object.assign(el.dataset, eventosPreservados[index].dataset);
                    el.className = eventosPreservados[index].className;
                }
            });
        }
    }
    ...
    document.addEventListener('click', function(e) {
        if (e.target.matches('.cambiar-estado-btn')) {
            const button = e.target;
            const pedidoId = button.getAttribute('data-pedido-id');
            const nuevoEstado = button.getAttribute('data-nuevo-estado');
            cambiarEstadoPedido(pedidoId, nuevoEstado, button);
        }
        
        if (e.target.closest('.card-mesa')) {
            const card = e.target.closest('.card-mesa');
            handleMesaCardClick(card);
        }
    });

    async function handleMesaCardClick(card) {
        const mesaId = card.getAttribute('data-mesa-id');
        const mesaNombre = card.getAttribute('data-mesa-nombre');
        const mesaUbicacion = card.getAttribute('data-mesa-ubicacion');
    ...
    function cambiarEstadoPedido(pedidoId, nuevoEstado, button) {
        fetch(`cambiar-estado-pedido/${pedidoId}/`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
